...
#[cfg(test)]
mod tests {
    use alphametics::{combinations::Combinations, others, slow, solver};
    use test::Bencher;
    ...
    fn bench_slow_dec_digits_combinations(b: &mut Bencher) {
        b.iter(|| {
            let mut num = slow::dec_digits_combinations::DecDigitsCombinations::new(DIGITS_COUNT).unwrap();
            while num.inc() {}
        });
    }
    ...
    fn bench_combinations_old(b: &mut Bencher) {
        b.iter(|| {
            let mut num = slow::combinations::Combinations::of_dec_digits(DIGITS_COUNT).unwrap();
            while num.inc() {}
        });
    }
