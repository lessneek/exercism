...
pub use dedupnum::DedupDecDigitNumber;
...
pub fn solve(input: &str) -> Option<HashMap<char, u8>> {
    ...
    while let Some(key_template) = num.next() {
        key.apply_value(key_template, &equation.charset);
...
impl CharKey {
    ...
    pub fn apply_value(&mut self, template: &[usize], charset: &[char]) {
        self.reset();

        for (i, &ch) in charset.iter().enumerate() {
            self.0[template[i]] = ch;
        }
    }
