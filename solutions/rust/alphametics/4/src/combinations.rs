...
pub fn combinations_of_dec_digits(size: usize) -> Option<Combinations<u8>> {
...
fn test_combinations_visual_output() {
    let mut num = combinations_of_dec_digits(8).unwrap();
    for i in 1usize.. {
        println!(
            "{0}: {1:?} {2} {3:?}",
            i,
...
fn test_combinations_for_dups() {
    for count in 1..=9 {
        let mut num = combinations_of_dec_digits(count).unwrap();
