...
const GLYPH_SQUARE: usize = GLYPH_HEIGHT * GLYPH_WIDTH;
...
pub fn convert(input: &str) -> Result<String, Error> {
    ...
    if let Some(row) = rows
        .iter()
        .find(|row| row.len() % GLYPH_WIDTH > 0 || row.len() != row_len)
    {
        return Err(InvalidColumnCount(row.len()));
    }

    let digit_glyphset = create_digit_glyphset();
    let mut result = String::with_capacity(input.len() / GLYPH_SQUARE);

    for ridx in 0..glyphs_rows_count {
        for gidx in 0..glyphs_in_row_count {
            let grow = &rows[ridx * GLYPH_HEIGHT..(ridx + 1) * GLYPH_HEIGHT];
            let glyph_raw: Vec<&str> = grow
                .iter()
                .map(|row| &row[gidx * GLYPH_WIDTH..(gidx + 1) * GLYPH_WIDTH])
                .collect();
            result.push(
                digit_glyphset
                    .recognize_raw(&glyph_raw)
