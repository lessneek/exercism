    ...
    fn update_dep_cells(&self, cell: &Cell<T>) -> Result<(), UpdateError> {
        ...
        if let Cell::Compute(compute_cell) = cell {
            ...
            if new_value != old_value {
                ...
                println!(
                    "COMPUTED CELL NOT UPDATED: id = {:?}, value = {:?}",
                    cell.id(),
                    compute_cell.value.get()
                );
            }
        ...
        if let Some(cell) = self.cells.get(&_id.0) {
            if let Cell::Input(input_cell) = cell {
                ...
                for cell in self.cells.values() {
                    if let Cell::Compute(cell) = cell {
                        if let Some(new_value) = cell.new_value.replace(None) {
                            if cell.value.replace(new_value) != new_value {
                                cell.callbacks
                                    .iter()
                                    .for_each(|(_, cb)| cb.borrow_mut()(cell.value.get()))
                            }
                        }
                    }
                }
